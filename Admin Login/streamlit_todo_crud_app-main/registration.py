...
def registration():
	...
	list_of_states = [i[1] for i in view_all_data_states()]
	
	list_of_courses = [i[3] for i in view_all_data_courses()]

	list_of_rooms = [i[2] for i in view_all_data_rooms()]
	...
	if choice == "Create":
		...
		with col1:
			id = st.text_input("Registration ID")
			roomno=st.selectbox("Room Number", list_of_rooms)
			seater=st.text_input("Sharing")
			feespm=st.text_input("Fees per month")
			foodstatus=st.selectbox("Food Availability", fdstatus)
			stayfrom=st.date_input("Stay From")
			duration=st.text_input("Duration")
			course=st.selectbox("Course", list_of_courses)
			...
			corresState=st.selectbox("Correspondence State",list_of_states)
			...
			pmnatetState=st.selectbox("Permanent State",list_of_states)
	...
	# 	# 	st.plotly_chart(p1,use_container_width=True)
		...
		if registration_id_result:
			...
			with col2:
				...
				new_corresState=st.selectbox("Correspondence State",list_of_states)
				...
				new_pmnatetState=st.selectbox("Permanent State",list_of_states)
