...
# course_id,course_name,course_format,singer_name,movie_name,movie_name,course_image,audio_file
	...
	# 	# 	st.plotly_chart(p1,use_container_width=True)
		...
		if course_id_result:
			...
			with col1:
				new_course_id = st.text_input("Course id",id)

			with col2:
				new_course_code = st.text_input("Course code",course_code)
				new_course_sn=st.text_input("Course short name",course_sn)
				new_course_fn=st.text_input("Course full name",course_fn)
		...
		delete_by_course_id =  st.selectbox("Select course ID",unique_list)
		if st.button("Delete"):
