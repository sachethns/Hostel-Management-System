...
def get_user_rgno(regNo):
	c.execute('SELECT * FROM userregistration WHERE regNo="{}"'.format(regNo))
	data = c.fetchall()
	return data
...
def get_room_number(room_no):
	c.execute('SELECT rooms.room_no, rooms.seater, rooms.fees, COUNT(roomno), rooms.seater - COUNT(roomno) FROM rooms INNER JOIN registration ON rooms.room_no=registration.roomno WHERE room_no="{}"'.format(room_no))
	data = c.fetchall()
	return data

def get_registration_detail(userEmail):
	c.execute('SELECT registration.id,registration.roomno,registration.seater,registration.feespm,registration.foodstatus,registration.stayfrom,registration.duration,registration.course,registration.regno,registration.firstName,registration.middleName,registration.lastName,registration.gender,registration.contactno,registration.emailid,registration.egycontactno,registration.guardianName,registration.guardianRelation,registration.guardianContactno,registration.corresAddress,registration.corresCIty,registration.corresState,registration.corresPincode,registration.pmntAddress,registration.pmntCity,registration.pmnatetState,registration.pmntPincode,registration.postingDate,registration.updationDate FROM registration INNER JOIN userlog ON registration.emailid=userlog.userEmail WHERE userEmail="{}"'.format(userEmail))
	data = c.fetchall()
	return data

def get_user_id_from_userlog(userId):
	c.execute('SELECT userId FROM userlog WHERE userId="{}"'.format(userId))
	data = c.fetchall()
	return data

def get_reg_number_from_userregistratiob(id):
	c.execute('SELECT regNo FROM userregistration WHERE id="{}"'.format(id))
	data = c.fetchall()
	return data

def get_registration_details(regno):
	c.execute('SELECT * FROM registration WHERE regno="{}"'.format(regno))
	data = c.fetchall()
	return data
